...
class HandStateReader:
    def __init__(self):
        ...
        self.subscriber = ChannelSubscriber(self.topic, MotorStates_)
        self.subscriber.Init(self.handler, 10)
        
        self.last_print = 0
        
    def handler(self, msg: MotorStates_):
        # Print at ~2Hz
        now = time.time()
    ...
    def print_state(self, msg: MotorStates_):
        # Clear line/screen or just print separator
        print("-" * 50)
        print(f"[G1 Hand State] (Timestamp: {time.time():.2f})")
